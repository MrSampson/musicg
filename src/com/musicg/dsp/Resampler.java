...
public class Resampler {
    ...
    /**
     * Do resampling. Currently the amplitude is stored by short such that
     * maximum bitsPerSample is 16 (bytePerSample is 2)
     * 
     * @param sourceData
     *            The source data in bytes
     * @param bitsPerSample
     *            How many bits represents one sample (currently supports max.
     *            bitsPerSample=16)
     * @param sourceRate
     *            Sample rate of the source data
     * @param targetRate
    ...
    public byte[] reSample(byte[] sourceData, int bitsPerSample,
        ...
        short[] amplitudes = new short[numSamples]; // 16 bit, use a short to
                                                    // store
        ...
        short[] targetSample = reSample.interpolate(sourceRate, targetRate,
                amplitudes);
        int targetLength = targetSample.length;
        // end do interpolation

        // TODO: Remove the high frequency signals with a digital filter,
        // leaving a signal containing only half-sample-rated frequency
        // information, but still sampled at a rate of target sample rate.
        // Usually FIR is used
