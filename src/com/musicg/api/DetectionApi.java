...
public class DetectionApi {
    ...
    public boolean isSpecificSound(byte[] audioBytes) {
        ...
        if (numSamples > 0 && Integer.bitCount(numSamples) == 1) {
            ...
            // to Nyquist theory
            unitFrequency = (double) waveHeader.getSampleRate() / 2
                    / numFrequencyUnit;
            ...
            double[][] spectrogramData = spectrogram
            ...
            System.arraycopy(spectrum, lowerBoundary, rangedSpectrum, 0,
                    rangedSpectrum.length);
        ...
        for (int i = 0; i < spectrogramData.length; i++) {
            for (int j = 0; j < spectrogramData[i].length; j++) {
                if (spectrogramData[i][j] < minValidAmp) {
                    ...
                    spectrogramData[i][j] = (Math.log10(spectrogramData[i][j]
                            / minAmp))
                            / diff;
                }
            }
    ...
    protected boolean isPassedStandardDeviation(double[][] spectrogramData) {
        ...
        double nthValue = arrayRankDouble.getNthOrderedValue(spectrum,
                numRobust, false);
    ...
    protected boolean isPassedFrequency(double[] spectrum) {
        ...
        ArrayRankDouble arrayRankDouble = new ArrayRankDouble();
        double robustFrequency = arrayRankDouble.getMaxValueIndex(spectrum)
                * unitFrequency;
